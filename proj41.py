        ...
        for playlist_id in playlist_ids:
            ...
            for video in videos:
                ...
                if video_details:
                    ...
                    data_youtube1.extend([{"video_details":data},{"video statistics":stat1data}])
                    data_youtube1.append("comments:")
                    st.subheader("\nComments:")

                    for comment in comments:
                        #data_youtube1.append({"comment_id":i})
                        ...
                        #data_youtube2.append(data1)
                    #data_youtube_list.append(data_youtube2)
...
def sql_comments(database, col_input):
    ...
    for doc in col.find({}, {'_id': 0, 'channel details': 1,}):
        ...
        for item in channel_details:
            # Check if the item is a dictionary (contains comment details)


            if isinstance(item, dict) and "video-id" in item:
                videos = item["video-id"]
                for video_data in videos:
                    # Check if the video_data is a dictionary (contains comment details)
                    if isinstance(video_data, dict):
                        # Check if the video_data has a "comments" key
                        if "comments" in video_data:
                            comments_data = video_data["comments"]
                            for comment in comments_data:
                                # Check if the comment is a dictionary (contains comment details)
                                if isinstance(comment, dict):
                                    comment_id = comment.get("comment_id", "")
                                    comment_text = comment.get("comment_text", "")
                                    comment_author = comment.get("comment_author", "")
                                    comment_published_date = comment.get("comment_published_date", "")
                                    comment_published_time = comment.get("comment_published_time", "")
    ...
    if s == m == []:
        ...
        if option_sql:
                ...
                cursor.executemany("""INSERT INTO video(video_id, video_name, video_description, published_date,
                                                    published_time, thumbnail,view_count,like_count, favourite_count, comment_count, duration,
                                                    caption_status) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
