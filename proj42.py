...
def sql_create_tables():
    ...
    cursor.execute("""CREATE TABLE IF NOT EXISTS video_stst(
                                            video_id            VARCHAR(255) PRIMARY KEY,
                                            view_count			INT,
                                            like_count			INT,
                                            favourite_count		INT,
...
def sql_videos(database, col_input):
    ...
    video_statistics = video_statistics.reindex(
                 ['video_id','view_count', 'like_count', 'favourite_count', 'comment_count', 'duration',
                  'caption_status'])
    ...
    return videos,video_statistics
...
def sql(database):
    ...
    if s == m == []:
        ...
        st.write(option_sql)
        if option_sql:
            col_input = "Google for Developers"
            ...
            videos, video_statistics = sql_videos(database, col_input)
