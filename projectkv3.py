...
def get_channel_details(youtube, CHANNEL_ID):
    try:
        ...
        channel_data={'Channel Title': {snippet['title']}}
        channel_data.update({'Channel Description': {snippet['description']},
                             'Channel Published Date': {snippet['publishedAt']},
...
def data_extraction_youtube(channel_name,CHANNEL_ID):
    ...
    if channel_name:
        ...
        for playlist_id in playlist_ids:
            ...
            #col.insertmany(data_youtube)
            for video in videos:

                video_id = video["snippet"]["resourceId"]["videoId"]
                st.write("Video_Id:", video_id)
                data_youtube1.update({"video_id":video_id})
                #col.insert_one(data_youtube)
...
def mongodb(database,channel,channel_name,channel_data,data_extraction):
...
def main():
  ...
  if st.button("Get Details"):
      ...
      mongodb(database,channel,channel_name,channel_data,data_extraction)
